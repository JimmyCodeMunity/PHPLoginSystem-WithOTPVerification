...
function sendVerificationEmail($email,$otp,$username,$uid) {
    ...
    try {
        ...
        $mail->SMTPAuth = true;
        $mail->Username = 'ghostbmer@gmail.com';
        $mail->Password = 'hozlcbspewvqxftn';
        $mail->SMTPSecure = 'tls';
        $mail->Port = 587;

        // Recipient
        $mail->setFrom('ghostbmer@gmail.com', 'Solve');
        $mail->addAddress($email);

        // Content
        $mail->isHTML(true); // Set to true to send HTML content
        $mail->Subject = 'Thanks for creating an account whi us ' .$username;
        ...
        $mail->send();
        header("location:verify.php?uid=$uid");
    } catch (Exception $e) {

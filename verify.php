...
$uid = $_GET['uid'];
$selection = "SELECT * FROM user WHERE uid = '$uid'";
...
if(isset($_POST['submit'])){
    ...
    $select = "SELECT * FROM user WHERE uid = '$uid' && otp = '$enteredotp'";
...
if(isset($_POST['resend'])){
    ...
    if($selected){
        $update = "UPDATE user SET otp = '$newotp',expirationtime='$formattedExpirationTime' WHERE uid = '$uid'";
        $succupdate = mysqli_query($conn,$update);
        $updated = "SELECT * FROM user WHERE uid = '$uid'";
...
<body class="bg-white">
  ...
  <form class="form" method="post" action="">
      <p class="login">Enter OTP to verify</p>
      <div class="inputContainer">
        ...
        <br>
        <br>
        <input type="submit" name="resend" value="Resend" class="submit2">
